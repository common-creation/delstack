...
func TestCdkAction_Validation(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			action:  NewCdkAction([]string{"Stack1"}, "", "", true, false, true, 0, false, false, "./cdk.out", nil),
			wantErr: "InvalidOptionError",
		},
		{
			name:    "negative concurrency number",
			action:  NewCdkAction(nil, "", "", false, false, true, -1, false, false, "./cdk.out", nil),
			wantErr: "InvalidOptionError",
		},
	}
...
func TestCdkAction_NoCdkJson(t *testing.T) {
	...
	action := NewCdkAction(nil, "", "", false, false, true, 0, false, false, "", nil)
...
func TestCdkAction_AppPathNoManifest(t *testing.T) {
	...
	action := NewCdkAction(nil, "", "", false, false, true, 0, false, false, tmpDir, nil)
	err := action.Run(context.Background())
	if err == nil {
		t.Fatal("expected error, got nil")
	}
...
func TestCdkAction_StackNameNotInManifest(t *testing.T) {
	...
	action := NewCdkAction([]string{"NonExistentStack"}, "", "us-east-1", false, false, true, 0, false, false, tmpDir, nil)
	err = action.Run(context.Background())
	if err == nil {
		t.Fatal("expected error, got nil")
	}
...
func TestCdkAction_AppPathCommand(t *testing.T) {
	...
	action := NewCdkAction(nil, "", "", false, false, true, 0, false, false, "echo hello", nil)
	err := action.Run(context.Background())
	if err == nil {
